...
describe("GetFilesWithViolationsInteractor", () => {
    ...
    it("returns the file paths of nodes with VIOLATION status", async () => {
        ...
        const interactor = new GetFilesWithViolationsInteractor(genericDBAccess, outputData);

        await interactor.execute();

        expect(outputData.result.files).toEqual(
            expect.arrayContaining(["src/entities/Student.java", "src/use_case/EnrollStudent.java"])
        );
    ...
    it("excludes nodes that are not in VIOLATION status", async () => {
        ...
        expect(outputData.result.files).toEqual(["src/interface_adapters/BadController.java"]);
    });
